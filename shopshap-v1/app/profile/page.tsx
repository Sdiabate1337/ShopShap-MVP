...
const menuItems = [
  ...
  { label: 'Catalogue', route: '/products', icon: '📦' },
  { label: 'Commandes', route: '/orders', icon: '📋' },
  { label: 'Profil', route: '/profile', icon: '👤', active: true },
];
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      const { error: deleteError } = await supabase
        .from('shops')
        .update({ 
          name: '[COMPTE SUPPRIMÉ - 2025-07-01 02:56:19]',
          activity: '[SUPPRIMÉ]',
          city: '[SUPPRIMÉ]',
          description: 'Compte supprimé le 2025-07-01 02:56:19 UTC par Sdiabate1337'
        })
        .eq('id', shop.id);
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-night via-night to-blue-950">
        <div className="text-center">
          ...
          <p className="text-white/60 text-xs mt-2">Connecté : Sdiabate1337</p>
        </div>
      </div>
    );
  }
          ...
          <div className="space-y-6">
            ...
            <div className="bg-night-foreground/10 border border-night-foreground/20 rounded-xl p-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div>
                  <span className="text-night-foreground/70">Session :</span>
                  <div className="text-green-400 font-medium text-xs">Sdiabate1337 • Actif</div>
                </div>
              </div>
            </div>
          </div>
